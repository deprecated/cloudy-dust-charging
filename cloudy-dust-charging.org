  ...
  for star, L4, cmap in stars:
      ...
      for denid, col in zip(denids, cols):
          ...
          for grain in m.data['gdrift'].colnames[1::5]:
              ax.plot(Prad/Pgas, m.data['gdrift'][grain], alpha=0.7, lw=0.5, color=col)
      ...
      for star, L4, cmap in stars:
          ...
          for denid, col in zip(denids, cols):
              ...
              ax.plot(G_n[mm & msub], m.data['gdrift'][grain][mm & msub],
                      alpha=0.7, lw=0.5, color=col, label=f"{star} {denid}")
              ax.plot(G_n[mm & ~msub], m.data['gdrift'][grain][mm & ~msub],
                      alpha=0.7, lw=0.5, color=col, label="_nolabel_")
      ax.set_title(grain, fontsize="x-small")

  axes[0, 0].legend(ncol=2, fontsize="xx-small", loc="upper left")
  axes[1, 0].set(
      yscale='log',
      xscale='log',
      xlabel='$G / n$',
