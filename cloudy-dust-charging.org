...
+ First of all, look at \tau_\nu in the final zone bu using the continuum
...
#+BEGIN_SRC python :return pltfile :results file
  ...
  for star, L4 in stars:
      ...
      for denid in denids:
          ...
          # F_nu spectrum that is normalized to unity
          Fnu_0 = nuFnu_inc / nu
          Fnu_0 /= Fnu_0.sum()
          ...
          tau_rscale_euv = np.array([np.trapz(nH0[:i], depth[:i]) for i in len(nzones)])
        
          for radius in m.data['rad']['radius']:
              Fnorm = L / (4*p.pi*radius**2)
              Fnu = np.emptylike(Fnu_0)
              Fnu[mfuv] = 
