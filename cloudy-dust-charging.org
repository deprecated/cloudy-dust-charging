...
* TODO Rerun the generic models but saving IR emissivities
+ [2019-02-08 Fri] I want to compare the dust emission with the strange models that Kobulnicky are using, to see how important it is that they are using completely the wrong incident SED
...
** TODO Look at grain emissivity and T versus radiation field
+ This is for comparison with the Kobulnicky papers, where they used Draine models that are meant for the ISRF, not O stars
+ Plan:
  1. [ ] Look at 70 micron emissivity versus radiation field U
  2. [ ] Look at grain T vs U (but this will be different for different sizes)
  3. [ ] Look at flux ratios between bands: 70/24, 24/8, 160/70
  ...
  for star, L4, cmap in stars:
      ...
      for denid, col in zip(denids, cols):
          ...
          for grain in m.data['gtemp'].colnames[1::5]:
              ax.plot(U, m.data['gtemp'][grain], alpha=0.7, lw=0.5, color=col)

  ax.set(
      yscale='log',
      xscale='log',
      xlabel='U',
      ylabel=r'Grain Temperature',
      #xlim=[1.0, 1.e6],
  )
